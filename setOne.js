...
// CODE HERE
const printParameters = (parameter1, parameter2) => console.log(`The first parameter is ${parameter1}. The second parameter is ${parameter2}.`);



// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.

printParameters("value1", "value2");
...
greeting("Chris", "Wallis" , function(fullName){
...
function pricesPlusTax(prices, callback){
    ...
    for (let i=0; i < prices.length; i++){
        ...
        let tax = 0.2 * originalPrice;
        let totalPrice = originalPrice + tax;
        totalCost.push(totalPrice);
    }
    callback(totalCost)
}
pricesPlusTax(prices, function(totalCost){
    console.log("Total costs with tax:", totalCost);
});
/* 
